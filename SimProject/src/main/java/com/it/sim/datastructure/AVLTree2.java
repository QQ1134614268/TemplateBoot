...
/**
 * 平衡二叉树 定义:首先它是一种特殊的二叉排序树，其次它的左子树和右子树都是平衡二叉树， 且左子树和右子树的深度之差不超过1
 * 平衡因子:可以定义为左子树的深度减去右子树的深度
 * <p>
 * 平衡二叉树是对二叉排序树的优化，防止二叉排序树在最坏情况下平均查找时间为n， 二叉排序树在此时形如一个单链表
...
public class AVLTree2<E extends Comparable<E>> {
    ...
    /**
     * @param p 最小旋转子树的根节点 向左旋转之后p移到p的左子树处，p的右子树B变为此最小子树根节点， B的左子树变为p的右子树 比如： A(-2)
    ...
    /**
     * @param p 最小旋转子树的根节点 向右旋转之后，p移到p的右子节点处，p的左子树B变为最小旋转子树的根节点 B的右子节点变为p的左节点、 例如:
    ...
    /**
     * 平衡而二叉树的插入操作 基本原理为： 1.首先如同二叉排序树一般，找到其要插入的节点的位置，并把元素插入其中； 2.自下向上进行回溯，回溯做两个事情：
     * (1)其一是修改祖先节点的平衡因子，当插入 一个节点时只有根节点到插入节点 的路径中的节点的平衡因子会被改变，而且改变的原则是当插入节点在某节点(称为A)
     * 的左子树 中时，A的平衡因子(称为BF)为BF+1,当插入节点在A的右子树中时A的BF-1，
     * 而判断插入节点在左子树中还是右子树中只要简单的比较它与A的大小 (2)在改变祖先节点的平衡因子的同时，找到最近一个平衡因子大于2或小于-2的节点，
     * 从这个节点开始调整最小不平衡树进行旋转调整，关于如何调整见下文。 由于调整后，最小不平衡子树的高度与插入节点前的高度相同，故不需继续要调整祖先节点。
    ...
    /**
     * 调整的方法： 1.当最小不平衡子树的根(以下简称R)为2时，即左子树高于右子树： 如果R的左子树的根节点的BF为1时，做右旋；
     * 如果R的左子树的根节点的BF为-1时，先左旋然后再右旋
     * <p>
     * 2.R为-2时，即右子树高于左子树： 如果R的右子树的根节点的BF为1时，先右旋后左旋 如果R的右子树的根节点的BF为-1时，做左旋
     * <p>
     * 至于调整之后，各节点的BF变化见代码
     */
    ...
    /**
     * 做左平衡处理 平衡因子的调整如图： t rd / \ / \ l tr 左旋后右旋 l t / \ -------> / \ / \ ll rd ll
     * rdl rdr tr / \ rdl rdr
     * <p>
     * 情况2(rd的BF为0)
     * <p>
     * <p>
     * t rd / \ / \ l tr 左旋后右旋 l t / \ -------> / \ \ ll rd ll rdl tr / rdl
     * <p>
     * 情况1(rd的BF为1)
     * <p>
     * <p>
     * t rd / \ / \ l tr 左旋后右旋 l t / \ -------> / / \ ll rd ll rdr tr \ rdr
     * <p>
     * 情况3(rd的BF为-1)
     * <p>
     * <p>
    ...
    /**
     * 做右平衡处理 平衡因子的调整如图： t ld / \ / \ tl r 先右旋再左旋 t r / \ --------> / \ / \ ld rr tl
     * ldl ldr rr / \ ldl ldr 情况2(ld的BF为0)
     * <p>
     * t ld / \ / \ tl r 先右旋再左旋 t r / \ --------> / \ \ ld rr tl ldl rr / ldl
     * 情况1(ld的BF为1)
     * <p>
     * t ld / \ / \ tl r 先右旋再左旋 t r / \ --------> / / \ ld rr tl ldr rr \ ldr
     * 情况3(ld的BF为-1)
     * <p>
    ...
    /**
     * 删除某节点p后的调整方法： 1.从p开始向上回溯，修改祖先的BF值，这里只要调整从p的父节点到根节点的BF值，
     * 调整原则为，当p位于某祖先节点(简称A)的左子树中时，A的BF减1，当p位于A的
     * 右子树中时A的BF加1。当某个祖先节点BF变为1或-1时停止回溯，这里与插入是相反的， 因为原本这个节点是平衡的，删除它的子树的某个节点并不会改变它的高度
     * <p>
