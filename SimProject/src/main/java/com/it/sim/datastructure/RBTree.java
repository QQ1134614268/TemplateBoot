...
public class RBTree<T extends Comparable<T>> {
    ...
    private void insertFixUp(RBNode<T> node) {
        RBNode<T> parent, pParent; // 定义父节点和祖父节点

        // 需要修整的条件：父节点存在，且父节点的颜色是红色
        while (((parent = parentOf(node)) != null) && isRed(parent)) {
            pParent = parentOf(parent);// 获得祖父节点

            // 若父节点是祖父节点的左子节点，下面else与其相反
            if (parent == pParent.left) {
                RBNode<T> uncle = pParent.right; // 获得叔叔节点

                // case1: 叔叔节点也是红色
                if (uncle != null && isRed(uncle)) {
                    setBlack(parent); // 把父节点和叔叔节点涂黑
                    setBlack(uncle);
                    setRed(pParent); // 把祖父节点涂红
                    node = pParent; // 将位置放到祖父节点处
    ...
    /*
     * key---节点的键值 direction--- 0:表示该节点是根节点 1:表示该节点是它的父节点的左子节点 2:表示该节点是它的父节点的右子节点
     */
